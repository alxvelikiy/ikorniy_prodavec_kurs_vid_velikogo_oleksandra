...
for (const s of steps) {
  ...
  if (s.result && fs.existsSync(path.join(RES, s.result))) {
    ...
    Object.assign(row, { passed: j.passed, failed: j.failed, file: 'v2/mvp/tests/results/' + s.result });
...
console.log('\n=== ПІДСУМОК ===');
for (const s of summary.steps) console.log(`${s.exit === 0 ? '✓' : '✗'} ${s.name}${s.passed != null ? ` — ${s.passed} ok, ${s.failed} fail` : ''} (${s.seconds} с)`);
console.log(`РАЗОМ: ${summary.passed} перевірок пройдено, ${summary.failed} не пройдено`);
