...
(function () {
  ...
  // ---------- SOS: живий фільтр карток (текст + категорія разом) ----------
  if (KIND === 'sos') {
    var sosInput = document.getElementById('sos-filter');
    var sosCards = [].slice.call(document.querySelectorAll('.say-card'));
    var sosChips = [].slice.call(document.querySelectorAll('.sos-chip'));
    var activeCat = null;

    function applySosFilter() {
      var st = stems(sosInput ? sosInput.value : '');
      var shown = 0;
      sosCards.forEach(function (card) {
        var t = norm(card.textContent + ' ' + (card.getAttribute('data-keys') || ''));
        var okText = !st.length || st.every(function (w) { return t.indexOf(w) >= 0; });
      ...
      if (!shown) {
        if (!empty && sosCards.length) {
          empty = el('p', 'sos-empty', 'Нічого не знайдено. Спробуй інше слово або скинь категорію.');
    ...
    if (sosInput && sosCards.length) {
      sosInput.addEventListener('input', applySosFilter);
    }
    sosChips.forEach(function (chip) {
      chip.addEventListener('click', function () {
        var cat = chip.getAttribute('data-cat');
        activeCat = activeCat === cat ? null : cat;
        sosChips.forEach(function (c) { c.classList.toggle('active', c.getAttribute('data-cat') === activeCat); });
        applySosFilter();
        if (activeCat) chip.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
      });
    });
  }

  // ---------- Бібліотека дзвінків: той самий фільтр, тільки за критерієм K ----------
  if (KIND === 'dzvinky') {
    var callInput = document.getElementById('call-filter');
    var callCards = [].slice.call(document.querySelectorAll('.call-card'));
    var callChips = [].slice.call(document.querySelectorAll('.sos-chip'));
    var activeK = null;

    function applyCallFilter() {
      var st = stems(callInput ? callInput.value : '');
      var shown = 0;
      callCards.forEach(function (card) {
        var t = norm(card.textContent);
        var okText = !st.length || st.every(function (w) { return t.indexOf(w) >= 0; });
        var crit = (card.getAttribute('data-criteria') || '').split(' ');
        var okK = !activeK || crit.indexOf(activeK) >= 0;
        var ok = okText && okK;
        card.classList.toggle('is-hidden', !ok);
        if (ok) shown++;
      });
      var empty = document.getElementById('call-empty-msg');
      if (!shown) {
        if (!empty && callCards.length) {
          empty = el('p', 'sos-empty', 'Нічого не знайдено. Спробуй інше слово або скинь критерій.');
          empty.id = 'call-empty-msg';
          callCards[callCards.length - 1].parentNode.appendChild(empty);
        }
      } else if (empty) { empty.remove(); }
    }

    if (callInput && callCards.length) callInput.addEventListener('input', applyCallFilter);
    callChips.forEach(function (chip) {
      chip.addEventListener('click', function () {
        var k = chip.getAttribute('data-cat');
        activeK = activeK === k ? null : k;
        callChips.forEach(function (c) { c.classList.toggle('active', c.getAttribute('data-cat') === activeK); });
        applyCallFilter();
      });
    });
  }
  ...
  if (lesson && /urok|day|vstup|chzv/.test(KIND)) {
    ...
    var plaque = document.querySelector('.cover-plaque');
    if (plaque) plaque.appendChild(el('span', 'readtime', '≈ ' + Math.max(1, Math.round(words / 170)) + ' хв читання'));
  }
